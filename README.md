...
Enable local voice during first-time setup, then start the voice channel:
...
Use `api` for HTTP JSON, WebSocket, and the built-in web page. Use `feishu` for the Feishu bot. Use `voice` for the local microphone/speaker channel.
...
Memory is an asynchronous derivative of the message stream. Complete raw conversations and observations live in `messages/`; long-term memory is rebuilt later from the persisted message history, covering the new memory-worthy period since the last journal checkpoint. Journaling is idle-first: when unwritten memory-worthy messages have been quiet for 30 minutes, the runtime writes a diary entry for that period. Long-lived API, Feishu, and Weixin runtimes use heartbeat ticks to check that due condition; foreground CLI exits still do not block on memory generation. Recent memory context is managed automatically and defaults to the last 2 days. Long-term memory is time-based and stored as daily, weekly, monthly, and yearly markdown files.
