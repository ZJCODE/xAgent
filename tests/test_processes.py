...
from xagent.interfaces.processes import pid_is_running, running_pid


class ManagedProcessTests(unittest.TestCase):
    def test_pid_is_running_treats_zombie_process_as_stopped(self):
        with patch("xagent.interfaces.processes.os.kill", return_value=None):
            with patch(
                "xagent.interfaces.processes.subprocess.run",
