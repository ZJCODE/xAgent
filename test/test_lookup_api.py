...
def test_lookup_word_anonymous_save():
    response = client.post("/lookup", json={"word": "anonymous_test", "save": True})
    assert response.status_code == 200
    data = response.json()
...
def test_lookup_word_extra_field():
    # extra 字段全部转为字符串
    response = client.post("/lookup", json={"word": "extra_test", "user_id": "user_extra", "save": True, "extra": {"tag": "pytest_extra", "level": "2"}})
    assert response.status_code == 200
    data = response.json()
...
def test_lookup_word_cache():
    # 第一次查词
    response1 = client.post("/lookup", json={"word": "cache_test", "user_id": "user_cache", "save": True})
    assert response1.status_code == 200
    data1 = response1.json()
    # 第二次查同一词，应该命中缓存/数据库，word/user_id 一致即可
    response2 = client.post("/lookup", json={"word": "cache_test", "user_id": "user_cache", "save": True})
...
def test_lookup_word_disable_cache():
    ...
    response = client.post("/lookup", json={"word": "no_cache_test", "user_id": "user_no_cache", "cache": False})
    assert response.status_code == 200
    data = response.json()
    assert "word" in data
