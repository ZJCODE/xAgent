...
- `runtime`：可选，运行时行为配置，例如默认 IANA 时区。
...
原始事件流和长期记忆统一保存在 `memory/xagent_memory.sqlite3`。核心表包括 `events`、`memory_items`、`memory_evidence`、`memory_revisions`、`people`、`memory_summaries` 和 `retention_policies`，并通过 FTS5 为事件和记忆内容建立全文索引。模型不直接执行 SQL；普通回忆使用 `recall_memory`，深翻原始历史使用 `search_history`，写入、纠错和遗忘分别使用 `remember`、`correct_memory`、`forget_memory`。
...
- `timezone`：可选，IANA 时区名；用于本轮 prompt 时间、历史时间和消息查询工具的本地化展示。
...
```json
{
  ...
  "memory_db": "/Users/you/.xagent/memory/xagent_memory.sqlite3",
  "message_storage": {
    "stream": "experience_memory",
    "backend": "sqlite",
    "path": "/Users/you/.xagent/memory/xagent_memory.sqlite3"
  },
  "tools": ["run_command", "web_search", "remember", "recall_memory", "search_history", "correct_memory", "forget_memory"],
  "identity": "# Identity\n\nYou are a helpful assistant.",
...
```json
{
  ...
  "events": 120,
  "memory_items": 42,
  "active_memory_items": 36,
  "candidate_memory_items": 4,
  "summaries": 3,
  "people": 8
}
```
...
```http
GET /api/memory/recall?query=路线图&max_items=8&include_evidence=false
```

#### GET /api/memory/items

分页读取结构化记忆，可按 `status`、`kind`、`subject_type` 过滤。

```http
GET /api/memory/items?status=active&limit=50&offset=0
```
...
```http
GET /api/messages?count=50&offset=0&timezone=Asia/Shanghai
```
...
- `count`：`1` 到 `500`，默认 `50`。
- `offset`：跳过最近多少条，默认 `0`。
- `timezone`：可选，IANA 时区名，用于派生人类可读时间；未提供时使用 `runtime.timezone`，再回退到系统本地时区/UTC。
  ...
  "messages": [
    {
      ...
      "timestamp_utc": "2025-10-09 08:53:20 UTC (+00:00)",
      "timestamp_local": "2025-10-09 16:53:20 Asia/Shanghai (+08:00)",
      "timezone": "Asia/Shanghai",
      "utc_offset": "+08:00",
      "metadata": {}
    }
...
返回消息数量、存储后端、默认显示时区和最早/最新 UTC Unix timestamp 的 UTC/本地格式化结果。
...
`memory_items(kind='person_fact')` 是人物事实层。只有长期记忆写入成功后，系统才会尝试从同一批消息中抽取有明确人物归属、带证据 quote 的稳定信息并追加到该层。人物事实更新失败不会影响主记忆写入。
