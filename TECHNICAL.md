...
```yaml
runtime:
  default_channel: api      # 可选：api | feishu
...
`channels.voice` 顶层键：`provider`（`soniox` / `qwen` / `custom`）、`websocket_base_url`、`enable_interruptions`（默认 `false`）、`audio`、`wake`、`stt`、`tts`。API key 只配置在 `stt.api_key` 和 `tts.api_key`；即使 STT 和 TTS 使用同一个 provider，也保持同一种结构。当 `provider` 不是 `custom` 时，`stt.provider` 与 `tts.provider` 必须与顶层 `provider` 一致。
...
调度器使用运行目录下的 `tasks/` 作为唯一状态队列。计划任务使用 `YYYYMMDD-HHMMSS-xxxxxxxx.json`，文件名编码当前生效的下一次触发时间，内容是结构化任务 envelope，主要字段包括 `kind=task`、稳定 `id`、`title`、`task.type`（`message` 或 `agent`）、`task.content`、`delivery.channel`、`delivery.target`、`execution`（`history_count` / `max_iter` / `max_concurrent_tools`）。周期规则统一写入 `recurrence` 规则列表，例如 `[{"kind":"daily","time":"10:00:00"}]` 或 `[{"kind":"weekly","time":"14:28:00","weekdays":["wed","fri"]}]`。
...
xagent voice [--dir DIR] [--user-id ID] [--verbose] [--list-devices]
             [--input-device DEV] [--output-device DEV]
...
Agent 侧最多注册以下 function tools（按 `TOOL_POLICY_ORDER` 排序）：
...
| `write_memory` | 始终加载 | 写入长期记忆（追加到当日 daily 文件）。 |
| `search_memory` | 始终加载 | 按关键词、日期或日期范围检索长期记忆，参数含 `query`、`date`、`scope`（`daily`/`weekly`/`monthly`/`yearly`/`all`）、`context_lines`（0–10）。 |
...
每轮对话会自动注入最近 `2` 天的 daily 记忆作为上下文。记忆行为始终开启：
...
- 敏感会话：当前始终读取和写入长期记忆；如需临时私密会话，建议在对话中明确告知模型不要调用 `write_memory` 工具。消息流始终持久化存储。
