...
用于记录“观察到的上下文”，再由 Agent 判断是否需要回复。适合群聊中未 @ 机器人的消息、环境传感器、通知、语音转写、房间状态等。
...
- `context`：必填，观察内容。
- `current_user_id`：可选，当前附近或活跃用户，默认 `default_user`。
- `source`：可选，事件来源，默认 `environment`。
- `event_type`：可选，事件类型，默认 `observation`。
- `sender_id`：可选，事件发出者或被听到的说话人。
- `metadata`：可选，外部系统元数据。
- 其他参数与 `/chat` 含义一致。
...
Agent 会基于对话和观察事件异步整理日记式记忆。`enable_memory` 和 `private` 会影响记忆行为：
...
使用 `/observe`，让 Agent 判断是否需要插话：

```json
{
  "context": "群聊中 Bob 说：下午的演示提前到三点。",
  "current_user_id": "feishu_open_id_alice",
  "source": "feishu",
  "event_type": "group_message",
  "sender_id": "feishu_open_id_bob",
  "metadata": {
    "chat_id": "oc_xxx",
    "message_id": "om_xxx",
    "addressed_to_agent": false
...
- 群聊：`sender_id` 记录真实发言人；`current_user_id` 记录当前触发交互或机器人正在服务的人。
