...
class ModelClient:
    ...
    def _build_responses_create_params(
        ...
        if tool_specs:
            ...
            params["tool_choice"] = "auto"
            # DeepSeek Responses API ignores `include`; OpenAI uses encrypted
            # reasoning content for multi-turn tool replay.
            if self.provider_name != PROVIDER_DEEPSEEK:
                params["include"] = ["reasoning.encrypted_content"]
        if self.max_tokens is not None:
            params["max_output_tokens"] = self.max_tokens
        if self.reasoning is not None:
            # DeepSeek Responses maps effort none/low/high/max (xhigh→high).
            params["reasoning"] = {
        ...
        for message in messages:
            ...
            if message_type in {"reasoning", "function_call", "function_call_output", "message"}:
                ...
                if isinstance(item, dict):
                    item = self._sanitize_responses_input_item(item)
    ...
    def _merge_responses_stream_event(
        ...
        if event_type == "response.output_item.done":
            # DeepSeek (and OpenAI) finalize reasoning/message items here with
            # full plain-text content needed for multi-turn tool replay.
            raw_item = cls._field(event, "item") or {}
            item = cls._to_plain_data(raw_item)
        ...
        if event_type == "response.reasoning_text.delta":
            # Accumulate DeepSeek chain-of-thought into the reasoning output item.
