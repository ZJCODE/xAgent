...
from ...application import create_runtime_heartbeat
from ...bootstrap.container import BaseAgentConfig, BaseAgentRunner
...
def handle_voice(args: argparse.Namespace) -> int:
    ...
    try:
        if getattr(args, "list_devices", False):
            from ...infrastructure.voice.audio import list_audio_devices_text
...
def handle_server(args: argparse.Namespace) -> int:
    from ...api import AgentHTTPServer
...
def _channel_command(channel: str, args: argparse.Namespace) -> list[str]:
    command = [sys.executable, "-m", "xagent.cli", "_run-channel", channel]
...
def _run_feishu_channel(args: argparse.Namespace, config: dict[str, Any]) -> int:
    ...
    try:
        from ...channels.feishu import FeishuAdapter, FeishuAdapterConfig
...
def _run_weixin_channel(args: argparse.Namespace, config: dict[str, Any]) -> int:
    ...
    try:
        from ...channels.weixin import WeixinAdapter, WeixinAdapterConfig
    except ImportError as exc:  # pragma: no cover - defensive
...
def _run_voice_channel(args: argparse.Namespace, config: dict[str, Any]) -> int:
    ...
    try:
        ...
        from ...infrastructure.voice.config import VoiceChannelConfig
        from ...infrastructure.voice.factory import create_local_voice_runtime
        from ...channels.voice.runtime import VoiceRuntimeOptions
...
def handle_memory(args: argparse.Namespace) -> int:
    ...
    if args.memory_command == "list":
        from ...infrastructure.storage import MarkdownMemoryStore
